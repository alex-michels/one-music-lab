...
const roadmap = readFileSync(new URL('../ROADMAP.md', import.meta.url), 'utf8')
  .replace(/\r\n/g, '\n');
const curriculum = roadmap.split('## 6. Последовательность учебных выпусков\n')[1]
  ?.split('## 7. Источники:')[0];
...
await test('Every original topic is represented exactly once by a completion checkbox', () => {
  ...
  assert.equal(actual.length, expectedIds.length, 'No topic may be silently dropped or duplicated');
  const compareIds = (left, right) => left.localeCompare(right, 'en');
  assert.deepEqual([...actual].sort(compareIds), [...expectedIds].sort(compareIds));
...
await test('Each curriculum module has four delivery layers and separate ready/published gates', () => {
  ...
  for (const [, code, , body] of modules) {
    for (const label of ['Теория:', 'Лаборатория:', 'Тренажёр:', 'Энциклопедия:', 'G — готово к публикации:', 'Опубликовано владельцем:']) {
      assert.ok(body.includes(`**${label}**`), `${code} lacks ${label}`);
    }
    assert.match(body, /^- \[[ x]\] (\d+\.\d+|[A-D]\d+) — /m, `${code} must map original topics`);
    const sourceLine = body.match(/Источники для разработки: (S\d{2}(?:,S\d{2})*)\./)?.[1];
    assert.ok(sourceLine, `${code} needs source leads`);
    for (const source of sourceLine.split(',')) {
      assert.ok(roadmap.includes(`| ${source} |`), `${code} refers to unknown source ${source}`);
    }
  }
});
