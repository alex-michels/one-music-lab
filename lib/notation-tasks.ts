...
export const ornaments = [
  { figure: 'trill', title: nt('Trill', 'Трель', 'Triller') },
  {
    figure: 'mordent',
    title: nt('Lower mordent', 'Перечёркнутый мордент', 'Mordent'),
  },
  { figure: 'turn', title: nt('Turn', 'Группетто', 'Doppelschlag') },
  {
    figure: 'grace',
    title: nt(
      'Slashed grace note',
  ...
  { figure: 'after-grace', title: nt('After-grace', 'Нахшлаг', 'Nachschlag') },
  { figure: 'slide', title: nt('Slide ornament', 'Шлейфер', 'Schleifer') },
  {
    figure: 'arpeggio',
...
export function shortExcerpt(
  ...
  const parts = pitches.map((pitch, index) => {
    ...
    return {
      kind: 'pitch' as const,
      prompt: nt(`Note ${index + 1}`, `Нота ${index + 1}`, `Note ${index + 1}`)[
        lang
      ],
      answer,
      options: options(
